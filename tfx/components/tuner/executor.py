...
def _search(exec_properties: Dict[Text, Any],
            ...
            tuner_id: Optional[Text] = None) -> None:
  """Multi-process safe tuner.search()."""
...
def _maybe_parallel_search(exec_properties: Dict[Text, Any],
                           fn_args: fn_args_utils.FnArgs) -> None:
  """Launch tuner.search() loop possibly concurrently in multi-process."""
  ...
  p = multiprocessing.Pool(processes=num_parallel_trials)
  p.map(_search_wrapper, [(exec_properties, fn_args, 'tuner{}'.format(i))
...
class Executor(base_executor.BaseExecutor):
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    _maybe_parallel_search(exec_properties, fn_args)

    # Instantiate one more tuner in the main process, because tuner.search()
    # may have been instantiated and executed in a different process.
    tuner = _get_tuner_fn(exec_properties)(fn_args).tuner
